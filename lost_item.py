...
class LostItem:
    def __init__(self, id, category, brand, colour, description, date_found=None, status="found"):
        ...
        if date_found:
            self.date_found = datetime.fromisoformat(date_found)  # show the date in a nice way
        else:
            self.date_found = datetime.now()
        self.status = status

    def to_dict(self):                                            # converting object into a dictionary
        return {
            "id": self.id,
    ...
    @classmethod
    def from_dict(cls, data):                                  # converting dictionary into an object
        return cls(
            id=data["id"],
